...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(testChainConfig(), genesis, db, 5, func(i int, gen *BlockGen) {
		...
		case 0:
			...
			tx, _ := types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(10000), TxGas, nil, nil, addr1[0]).SignECDSA(key1)
			gen.AddTx(tx)
		case 1:
			// In block 2, addr1 sends some more ether to addr2.
			// addr2 passes it on to addr3.
			tx1, _ := types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(1000), TxGas, nil, nil, addr1[0]).SignECDSA(key1)
			tx2, _ := types.NewTransaction(gen.TxNonce(addr2), addr3, big.NewInt(1000), TxGas, nil, nil, addr2[0]).SignECDSA(key2)
			gen.AddTx(tx1)
			gen.AddTx(tx2)
		case 2:
